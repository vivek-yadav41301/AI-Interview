...
`;
    ...
    // Safely parse JSON even if Gemini includes markdown formatting
...
`;
    ...
    // FIXED: Corrected `questionsArray` typo to `questionArray` and `quetion` to `question`
    ...
    return res
      .status(500)
      .json({
        message: `Failed to create interview: ${error.message || error}`,
      });
  }
};
...
`,
    ...
    return res.status(500).json({
      message: `Failed to submit answer: ${error.message || error}`,
    });
  }
};
...
export const finishInterview = async (req, res) => {
  try {
    // FIXED: Added missing interview retrieval using interviewId
    ...
    return res.status(500).json({
      message: `Failed to finish interview: ${error.message || error}`,
    });
  }
};
...
export const getMyInterviews = async (req, res) => {
  try {
    const interviews = await Interview.find({ userId: req.userId }).select(
